      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 items-start">
        {tiers.map((tier, i) => (
          <div key={i} className={`glass rounded-3xl p-8 border transition-all hover:-translate-y-1 hover:shadow-2xl ${tier.popular ? 'border-indigo-500 scale-[1.03] z-10 bg-indigo-500/5' : 'border-white/5'}`}>
            ...
            <button className={`w-full py-4 rounded-xl text-[12px] font-black uppercase tracking-widest transition-all ${tier.popular ? 'bg-indigo-600 text-white hover:bg-indigo-500' : 'glass text-white hover:bg-white/5'}`}>
              {tier.button}
            </button>
          </div>
        ))}
